/*
  ...
  tandem bicycles as follows: [1, 4], [5, 3], then the total speed of these tandem
  bicycles is 4 + 5 = 9.
...
int tandemBicycle(vector<int> redSS, vector<int> blueSS, bool fastest) {
  sort(redSS.begin(), redSS.end());
  if (fastest) sort(blueSS.begin(), blueSS.end(), greater());
  else sort(blueSS.begin(), blueSS.end());

  int sum = 0;
  for (int i = 0; i < redSS.size(); i++) {
    sum += max(redSS[i], blueSS[i]);
  }
